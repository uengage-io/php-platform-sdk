 ...
 * `zones`, `wallet`, and `events` namespaces hanging off it.
 *
 * `events` is the odd one out: it publishes to the platform event bus
 * (SNS) with the host's AWS credentials rather than calling the platform
 * HTTP API, so it ignores the auth modes below entirely.
 *
 ...
 * UENGAGE_SESSION_ID, UENGAGE_SESSION_TOKEN, UENGAGE_ACTOR_VIA,
 * PLATFORM_EVENTS_TOPIC_ARN, PLATFORM_EVENTS_REGION and
 * PLATFORM_EVENTS_SOURCE are read from getenv(). Explicit input always
 * wins.
 */
class Client
{
    ...
    public function __construct(
        ...
        ?SnsPublisherInterface $snsPublisher = null
    ) {
        $this->config = $config;
        $this->http = $http;
        ...
        $this->events = new EventsClient($config, $snsPublisher);
    }
    ...
    public static function create(array $input = []): self
    {
        ...
        $config = new Config(
            ...
            $actorVia,
            null,
            self::pickOptional($input, 'eventsTopicArn', 'PLATFORM_EVENTS_TOPIC_ARN'),
            self::pickOptional($input, 'eventsRegion', 'PLATFORM_EVENTS_REGION'),
            self::pickOptional($input, 'eventSource', 'PLATFORM_EVENTS_SOURCE')
        );

        $snsPublisher = isset($input['snsPublisher'])
            && $input['snsPublisher'] instanceof SnsPublisherInterface
            ? $input['snsPublisher']
            : null;

        return new self($config, $http, $tokenSource, $snsPublisher);
    }
