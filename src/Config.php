...
class Config
{
     ...
     * under.
     *
     * VERSIONED IN LOCKSTEP WITH THE NODE SDK from 2.1.0 onward: the two
     * packages track the same platform API, and two numbers for one
     * contract made "which SDK version has the wallet passthroughs?" a
     * question with two answers. They had drifted by accident, not by
     * design — Node went 1.0.0 → 2.0.0 (its 1.1.0 tag failed to publish)
     * while PHP went 1.0.0 → 1.1.0.
     *
     * So this jumps 1.1.0 → 2.1.0, skipping 2.0.0. It is a major bump
     * carrying no breaking change, which has one real cost: composer
     * constraints pinned `^1.x` will not resolve it, so consumers must
     * widen to `^2.1` to pick it up.
     */
