...
class UpsellClient
{
     ...
     *                                customer?: {id?, contactMappingId?}, sessionId?}}
     ...
     *
     * `itemId` is ALWAYS the id the suggestion was served under. For an item
     * with variants that is the DISH, not the size the customer chose —
     * sending the size id leaves the `added` event unjoinable to the `shown`
     * that preceded it, so attach rate for every picker card reads zero.
     * Report the chosen size in `variantItemId`.
     *
     * @param array $input {channel, placement, tenant, sessionId?, configVersion?,
     *                      events: [{type: shown|added|ignored, itemId, reasonCode?,
     *                                variantItemId?, source?, occurredAt?}]}
     ...
     *
     * @param array $opts {parentId?: int|string, businessId?: int|string}
     * @return array list of PlacementRecord
     */
    public function adminListPlacements(array $opts = []): array
    {
        $opts = $this->normaliseScopeIds('adminListPlacements', $opts);
        $params = [];
        if (isset($opts['parentId'])) $params['parentId'] = $opts['parentId'];
        if (isset($opts['businessId'])) $params['businessId'] = $opts['businessId'];
     ...
     *
     * The server accepts only the literal 'true' or 'false' here; anything
     * else is a 400. Passing false is an explicit no, not consent — but omit
     * the key rather than sending false if you simply do not mean to delete
     * a default.
     *
