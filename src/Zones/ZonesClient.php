...
class ZonesClient
{
    ...
    /**
     * POST /v1/zones/nearby - radius search ordered by distance asc.
     *
     * @param array $input {point: {lat, lng}, radiusMeters: number, ids?: string[], tags?: array, limit?: int}
     * @return array<int, array> matching zones (each {zid, tags, distanceMeters})
     */
    public function nearby(array $input): array
    {
        if (!isset($input['point']) || !is_array($input['point'])) {
            throw new InvalidArgumentException('zones.nearby: input must include `point`');
        }
        $this->assertPoint('nearby', $input['point']);
        if (!isset($input['radiusMeters']) || !is_numeric($input['radiusMeters']) || $input['radiusMeters'] <= 0) {
            throw new InvalidArgumentException(sprintf(
                'zones.nearby: radiusMeters must be a number > 0 (got %s)',
                var_export($input['radiusMeters'] ?? null, true)
            ));
        }
        if (isset($input['ids'])) {
            if (!is_array($input['ids'])) {
                throw new InvalidArgumentException('zones.nearby: ids must be an array of uuid strings');
            }
            foreach ($input['ids'] as $zid) {
                $this->assertUuid('nearby', $zid);
            }
        }
        if (isset($input['limit']) && (!is_int($input['limit']) || $input['limit'] <= 0)) {
            throw new InvalidArgumentException(
                sprintf('zones.nearby: limit must be a positive integer (got %s)', var_export($input['limit'], true))
            );
        }
        $body = json_encode($input, JSON_UNESCAPED_SLASHES);
        $path = '/v1/zones/nearby';
        $response = $this->signer->send('POST', $path, $this->signer->url($path), $body);
        $decoded = $this->expectJson($response, 'nearby');
        return isset($decoded['zones']) && is_array($decoded['zones']) ? $decoded['zones'] : [];
    }
    ...
    /**
     * POST /v1/zones/intersecting - geometry overlap; non-empty result = overlap.
     *
     * @param array $input {geometry: GeoJsonPolygon|GeoJsonMultiPolygon, ids?: string[], tags?: array, excludeZid?: string}
     * @return array<int, array> matching zones (each {zid, tags})
     */
    public function intersecting(array $input): array
    {
        $this->assertGeometry('intersecting', $input);
        if (isset($input['ids'])) {
            if (!is_array($input['ids'])) {
                throw new InvalidArgumentException('zones.intersecting: ids must be an array of uuid strings');
            }
            foreach ($input['ids'] as $zid) {
                $this->assertUuid('intersecting', $zid);
            }
        }
        if (isset($input['excludeZid'])) {
            $this->assertUuid('intersecting', $input['excludeZid']);
        }
        $body = json_encode($input, JSON_UNESCAPED_SLASHES);
        $path = '/v1/zones/intersecting';
        $response = $this->signer->send('POST', $path, $this->signer->url($path), $body);
        $decoded = $this->expectJson($response, 'intersecting');
        return isset($decoded['zones']) && is_array($decoded['zones']) ? $decoded['zones'] : [];
    }
