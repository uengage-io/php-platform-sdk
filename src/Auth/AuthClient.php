...
class AuthClient
{
    /** Lifetime bounds the auth service enforces on the realtime-channel and upsell grants. */
    const MIN_TOKEN_TTL_SECONDS = 60;
    const MAX_TOKEN_TTL_SECONDS = 14400;
     ...
     */
    public function mintRealtimeToken(string $clientId, string $clientSecret, string $channel, int $expiresIn = self::DEFAULT_TOKEN_TTL_SECONDS, ?string $onBehalfOf = null): array
    {
        if ($clientId === '' || $clientSecret === '') {
        ...
        if ($expiresIn < self::MIN_TOKEN_TTL_SECONDS || $expiresIn > self::MAX_TOKEN_TTL_SECONDS) {
            throw new InvalidArgumentException(sprintf(
                'expiresIn must be between %d and %d seconds',
                self::MIN_TOKEN_TTL_SECONDS,
    ...
    /**
     * Backend only. Authorize the caller for the requested tenant, mapping or admin level.
     * Suggestion issuance requires upsell.tokens:issue; each admin level has its own capability.
     * @return array {token, expiresIn, parentId?, businessId?, access, contactMappingId?, adminScope?}
     */
    public function mintUpsellToken(string $clientId, string $clientSecret, ?int $parentId = null, ?int $businessId = null, string $access = 'public', ?int $contactMappingId = null, int $expiresIn = self::DEFAULT_TOKEN_TTL_SECONDS, ?string $adminScope = null): array
    {
        if ($clientId === '' || $clientSecret === '') {
            throw new InvalidArgumentException('Backend credentials are required');
        }
        if ($access === 'admin') $adminScope = $adminScope ?? 'outlet';
        if (($access !== 'admin' && $adminScope !== null)
            || ($access === 'admin' && !in_array($adminScope, ['outlet', 'brand', 'global'], true))
            || ($adminScope === 'global' ? ($parentId !== null || $businessId !== null) : ($parentId === null || $parentId <= 0 || $parentId > 9007199254740991))
            || (($adminScope === 'global' || $adminScope === 'brand') ? $businessId !== null : ($businessId === null || $businessId <= 0 || $businessId > 9007199254740991))) {
            throw new InvalidArgumentException('Invalid tenant IDs for the requested upsell/admin scope');
        }
        if (($access !== 'public' && $access !== 'private' && $access !== 'admin')
            || ($access !== 'private' && $contactMappingId !== null)
            || ($access === 'private' && ($contactMappingId === null || $contactMappingId <= 0 || $contactMappingId > 9007199254740991))) {
            throw new InvalidArgumentException('Private access requires a positive contactMappingId; public/admin access forbids it');
        }
        if ($expiresIn < self::MIN_TOKEN_TTL_SECONDS || $expiresIn > self::MAX_TOKEN_TTL_SECONDS) {
            throw new InvalidArgumentException(sprintf(
                'expiresIn must be between %d and %d seconds',
        ...
        $response = $this->http->send('POST', $this->config->getAuthBaseUrl() . '/oauth/token', [
            ...
            'grant_type' => 'urn:uengage:params:oauth:grant-type:upsell',
            'parent_id' => $parentId, 'business_id' => $businessId, 'access' => $access,
            'contact_mapping_id' => $contactMappingId, 'expires_in' => $expiresIn, 'admin_scope' => $adminScope,
        ]));
        if (!$response->isOk()) {
            throw new AuthApiException($response->getStatus(), 'Upsell token mint failed');
        }
        $body = $response->json();
        $expectedScope = $access === 'admin' ? 'upsell.placements:read upsell.placements:write' . ($adminScope === 'outlet' ? '' : ' upsell.touchpoints:read upsell.touchpoints:write') : 'upsell.suggestions:' . $access;
        if (!is_array($body) || !isset($body['access_token'], $body['expires_in'], $body['access'], $body['scope'], $body['token_type'])
            || !is_string($body['access_token']) || $body['access_token'] === '' || $body['token_type'] !== 'Bearer'
            || ($body['parent_id'] ?? null) !== $parentId || ($body['business_id'] ?? null) !== $businessId || $body['access'] !== $access
            || ($body['contact_mapping_id'] ?? null) !== $contactMappingId || ($body['admin_scope'] ?? null) !== $adminScope || $body['scope'] !== $expectedScope
            || !is_int($body['expires_in']) || $body['expires_in'] < 60 || $body['expires_in'] > $expiresIn) {
            throw new AuthApiException($response->getStatus(), 'Invalid upsell token response');
        }
        $result = ['token' => $body['access_token'], 'expiresIn' => $body['expires_in'], 'parentId' => $parentId, 'businessId' => $businessId, 'access' => $access];
        if ($contactMappingId !== null) $result['contactMappingId'] = $contactMappingId;
        if ($adminScope !== null) $result['adminScope'] = $adminScope;
        return $result;
    }
