...
class EventsClientTest extends TestCase
{
    ...
    public function testPublishBuildsTheEnvelope(): void
    {
        ...
        $this->assertSame(
            1,
            preg_match('/^[0-9A-HJKMNP-TV-Z]{26}$/', $envelope['id']),
            'envelope id must be a ULID, got ' . $envelope['id']
        );
    ...
    public function testEveryAcceptedOccurredAtMatchesTheEnvelopeSchema(): void
    {
        ...
        foreach ($publisher->allEnvelopes() as $envelope) {
            ...
            // php >=7.1 caps PHPUnit at 7.5. This is the convention for every
            // regex assertion in this suite — the four other call sites had
            // drifted onto the 9.1 assertion and were failing on 7.1
            // unnoticed, because the package had no CI job until
            // `ci-platform-sdk-php` was added. It runs 7.1/7.4/8.1, so that
